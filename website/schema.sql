...
CREATE TABLE IF NOT EXISTS bots (
  ...
  command_source TEXT NOT NULL DEFAULT 'developer_catalog' CHECK (command_source IN ('developer_catalog', 'custom')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX IF NOT EXISTS idx_bots_command_source ON bots(command_source);
...
-- ===== HARDENING =====
-- Un utilisateur ne peut pas s'attribuer lui-même un rôle admin, modifier son
-- identité de sécurité ou transformer son plan sans opération serveur contrôlée.
CREATE OR REPLACE FUNCTION protect_profile_security_fields()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'UPDATE' AND auth.uid() = OLD.id AND NOT EXISTS (
...
DROP TRIGGER IF EXISTS protect_profile_security ON profiles;
CREATE TRIGGER protect_profile_security
  BEFORE UPDATE ON profiles
  FOR EACH ROW EXECUTE FUNCTION protect_profile_security_fields();

-- Le client ne peut pas créer de session au nom d’un autre bot via une valeur
-- arbitraire : l’autorisation est vérifiée par la policy existante.

-- Migration à appliquer si la table bots existe déjà.
ALTER TABLE bots ADD COLUMN IF NOT EXISTS command_source TEXT NOT NULL DEFAULT 'developer_catalog';
DO $$ BEGIN
  ALTER TABLE bots ADD CONSTRAINT bots_command_source_check
    CHECK (command_source IN ('developer_catalog', 'custom'));
