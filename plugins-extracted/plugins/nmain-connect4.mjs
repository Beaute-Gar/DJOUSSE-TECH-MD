...
const connectfour = async (m, sock) => {
  ...
  if (cmd === "drop") {
    ...
    if (isNaN(col) || col < 0 || col > 6) return m.reply("âŒ Invalid column. Use 1-7.");
    ...
    if (!placed) return m.reply("âŒ Column is full.");
    ...
    if (checkWin(board, currentColor)) {
      ...
      delete games[from];
      return sock.sendMessage(from, {
        text: `ðŸ† *${winner} wins with ${currentColor}*\n\n${drawBoard(board)}`
      });
    }

    games[from].turn++;
    return sock.sendMessage(from, {
      text: `ðŸŽ® *Turn: ${games[from].players[games[from].turn % 2]}*\n\n${drawBoard(board)}`
    });
  }
