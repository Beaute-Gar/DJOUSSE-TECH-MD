...
const quiz = async (m, gss) => {
  try {
    ...
    if (quizSessions.has(sender)) {
      return m.reply("â³ You already have an active quiz. Reply to it first!");
    }
    ...
    if (!data.results || data.results.length === 0) {
      return m.reply("âŒ No quiz available right now. Try again later.");
    }
    ...
    const quizMessage = `*ðŸ§  QUIZ TIME*\n` +
    ...
    setTimeout(() => {
      if (quizSessions.has(sender)) {
        m.reply("â° Time's up! The quiz has expired.");
    ...
    console.error(err);
    m.reply("âŒ An error occurred while fetching the quiz.");
  }
};
