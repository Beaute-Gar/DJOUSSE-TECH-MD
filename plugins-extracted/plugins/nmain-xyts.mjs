  ...
  if (validCommands.includes(cmd)) {
    ...
    if (!searchQuery) {
      await gss.sendMessage(
        m.from,
        { text: "Nyaa~! âŒ You forgot to tell me what to search for! ðŸ¥º Please add a query after the command, pretty please~" },
        { quoted: m }
      );
      return;
    }
    ...
    try {
      ...
      if (apiData.status && apiData.result) {
        ...
        if (!videos || videos.length === 0) {
          await gss.sendMessage(
            m.from,
            { text: "Aww~ âŒ I couldnâ€™t find anything for that search... ðŸ˜¿ Please try something else, my dear!" },
            { quoted: m }
          );
          return;
        }
        ...
        let message = `*Yatta~! I found some super cute YouTube results for you!* ðŸ’–\n*â€œ${searchQuery}â€* ðŸŒ¸\n\n`;
        message += `~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n`;
        ...
        videos.slice(0, resultCount).forEach((video, index) => {
          message += `*${index + 1}. ${video.title}* ðŸŒŸ\n`;
          message += `â³ *Duration:* ${video.duration?.timestamp || "N/A"}\n`;
          message += `ðŸ‘€ *Views:* ${video.views || "Nyaa~ So many views, so cute!"}\n`;
          message += `ðŸ‘¤ *Author:* ${video.author?.name || "A mysterious creator~"}\n`;
          message += `ðŸ”— *[Watch here](https://youtube.com/watch?v=${video.videoId})*\n`;
          message += `~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n`;
        });
        ...
        const messagePayload = {
          ...
          contextInfo: {
            ...
            externalAdReply: {
              ...
              body: "á´˜á´á´¡á´‡Ê€á´‡á´… Ê™Ê DJOUSSSE",
              thumbnailUrl:
        ...
        await gss.sendMessage(
          m.from,
          { text: "Nyaa~ âŒ Something went wrong while fetching the videos... ðŸ˜¿ I'll try again soon, okay?!" },
          { quoted: m }
        );
      }
    } catch (error) {
      ...
      await gss.sendMessage(
        m.from,
        { text: "Waaah~! âŒ I ran into a lil' problem while searching... ðŸ˜¿ Please try again in a bit, pretty please?" },
        { quoted: m }
      );
    }
  }
