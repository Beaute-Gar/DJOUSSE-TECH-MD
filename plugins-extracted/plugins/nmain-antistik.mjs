  ...
  if (cmd === 'antisticker') {
    ...
    switch (subCmd) {
      case 'on':
        ...
        response = "ðŸ›¡ï¸ Anti-Sticker protection enabled\nStickers will be automatically deleted";
        break;
      
      case 'off':
        config.ANTI_STICKER = false;
        response = "ðŸ”“ Anti-Sticker protection disabled";
        break;
      
      case 'status':
        response = `Anti-Sticker Status: ${config.ANTI_STICKER ? 'ðŸŸ¢ ACTIVE' : 'ðŸ”´ INACTIVE'}`;
        break;
      
      default:
        response = `Anti-Sticker Commands:\n\nâ€¢ ${prefix}antisticker on - Enable protection\nâ€¢ ${prefix}antisticker off - Disable\nâ€¢ ${prefix}antisticker status - Show status`;
    }
