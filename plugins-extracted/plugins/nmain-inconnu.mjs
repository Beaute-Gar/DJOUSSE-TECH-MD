...
const ownerHandler = async (m, gss) => {
    ...
    if (cmd === 'inconnu') {
        ...
        if (!['add', 'del', 'list'].includes(action)) {
            await m.reply("Invalid usage. Try:\n.inconnu add 237693978044 (or reply)\n.inconnu del 237693978044 (or reply)\n.inconnu list");
            return;
        }
        ...
        if (action === 'add') {
            if (SUDO_LIST.includes(number)) {
                ...
                await m.reply(`âœ… ${number} has been added to the sudo list.`);
            }
        }
        ...
        if (action === 'del') {
            ...
            if (index !== -1) {
                ...
                await m.reply(`âŒ ${number} has been removed from the sudo list.`);
            } else {
