...
const wcg = async (m, sock) => {
  ...
  if (cmd === "resetwcg") {
    if (wcgGames[from]) {
      ...
      return m.reply("âŒ No active WCG game to reset.");
    }
  }
  ...
  if (game.attempts[sender].count >= 15) {
    return m.reply("âŒ You've used all 15 guesses! Wait for others or reset the game.");
  }
  ...
  if (guess === game.number) {
    ...
    return sock.sendMessage(from, {
      text: `ðŸ† *Correct!* ${sender} guessed the number *${guess}* in ${game.attempts[sender].count} tries!`,
    });
  } else {
    return m.reply(`âŒ ${guess} is wrong! (${15 - game.attempts[sender].count} guesses left)`);
  }
};
