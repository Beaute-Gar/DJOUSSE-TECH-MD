...
async function handleRawReply(sock, m) {
  const game = games.get(m.chat);
  if (!game || game.phase !== 'round' || !m.body) {
    console.log(`[AVT-RAW] SKIP chat=${m.chat} hasGame=${!!game} phase=${game?.phase} body=${!!m.body}`);
    return false;
  }
  const text = m.body.trim().toLowerCase();
  const sender = m.sender;

  console.log(`[AVT-RAW] chat=${m.chat} phase=${game.phase} text=${JSON.stringify(text)} sender=${sender} pending=${game.pendingChallenge.has(sender)} registered=${game.registered.has(sender)}`);
