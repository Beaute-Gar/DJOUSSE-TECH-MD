...
const pollCommand = async (m, Matrix) => {
  ...
  if (cmd === 'poll') {
    ...
    if (!rest.includes('|')) {
      return m.reply("âš ï¸ Usage: *poll Question | Option1 | Option2 | Option3*");
    }
    ...
    if (options.length < 2) {
      return m.reply("âš ï¸ You need at least *two options* for a poll.");
    }

    // Check for empty options
    if (options.some(option => option === "")) {
      return m.reply("âš ï¸ All poll options must be non-empty.");
    }
    ...
    try {
      ...
      m.reply(`âœ… Poll created successfully! Question: *${question}* with options: *${options.join(", ")}*`);
    } catch (error) {
      console.error("Error sending poll:", error);
      m.reply("âš ï¸ An error occurred while creating the poll.");
    }
  }
};
