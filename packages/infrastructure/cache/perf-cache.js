...
export function setRedisClient(client) {
  _redisClient = client;
  log.info('Redis client attached, TTL-driven caching active');
}
...
export class PerfCache {
  ...
  async getAsync(key) {
    ...
    if (_redisClient) {
      try {
        const raw = await _redisClient.get(`dj:${key}`);
        if (raw !== null) {
  ...
  set(key, value, ttl) {
    ...
    this.#stats.sets++;
    this._syncRedis(key, value, ttl);
    return value;
  }

  async _syncRedis(key, value, ttl) {
    if (!_redisClient) return;
    try {
      const t = ttl || this.#defaultTTL;
      await _redisClient.setEx(`dj:${key}`, Math.ceil(t / 1000), JSON.stringify(value));
    } catch {}
  }

  delete(key) {
    this.#store.delete(key);
    if (_redisClient) _redisClient.del(`dj:${key}`).catch(() => {});
  }
