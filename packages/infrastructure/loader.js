...
const CMDS_DIR = path.resolve(process.cwd(), 'src', 'commands');
...
export async function loadAllPlugins() {
  ...
  let dirExists = false, files;
  try { dirExists = (await stat(CMDS_DIR)).isDirectory(); } catch {}
  if (!dirExists) {
    log.warn(`Dossier commands introuvable : ${CMDS_DIR} — aucun plugin chargé`);
    isLoaded = true;
    loadedAt = new Date();
    return;
  }
