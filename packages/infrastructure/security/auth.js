...
export function getPrivilege(jid) {
  ...
  if (OWNER_JID && normalized.split('@')[0] === OWNER_JID.split('@')[0]) return 'owner';
  if (knownOwnerJids.has(normalized)) return 'owner';
  const num = normalized.split('@')[0];
  for (const kj of knownOwnerJids) { if (kj.split('@')[0] === num) return 'owner'; }
  if (SUDO_JIDS.has(normalized)) return 'sudo';
  for (const sj of SUDO_JIDS) { if (sj.split('@')[0] === num) return 'sudo'; }
...
function checkDbSudo(jid) {
  try {
    const { Sudo } = require('../database/database.js');
