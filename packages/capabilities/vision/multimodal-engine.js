...
import { bus, EVENTS } from '../../ainoria-intelligence/core/event-bus.js';
import { semanticMemory } from '../../ainoria-intelligence/memory/semantic-memory.js';
import { runtime } from '../../ainoria-intelligence/core/runtime.js';
...
export class MultimodalEngine {
  ...
  async _analyzeImage(input, co, options) {
    ...
    try {
      const { vision } = await import('./vision-engine.js');
      const result = await vision.analyze(input, { ...options, mimeType: co.mimeType, source: options.source });
      if (result?.text) {
        co.summary = result.text.slice(0, 300);
        co.content = result.text;
        co.entities = vision._extractEntities(result.text);
        co.concepts = vision._extractConcepts(result.text);
        co.metadata.visionModel = 'gemini-2.0-flash';
        co.metadata.visionDuration = result.duration;
        co.addStep({ name: 'vision_analyzed', duration: result.duration });
      } else {
        co.summary = result?.error || 'Analyse visuelle indisponible';
        co.addStep({ name: 'vision_error', note: result?.error });
      }
    } catch (err) {
      co.summary = 'Erreur analyse visuelle';
      co.addStep({ name: 'vision_error', error: err.message });
    }
  }
  ...
  async _analyzeAudio(input, co, options) {
    ...
    try {
      ...
      const result = await audio.transcribeAudio(input, { ...options, mimeType: co.mimeType, source: options.source });
      if (result?.transcription) {
        co.summary = result.summary || result.transcription.slice(0, 200);
        co.content = result.transcription;
        co.metadata.transcriptionModel = 'whisper+gemini';
        co.metadata.language = result.language;
        co.metadata.urgency = result.urgency;
        co.metadata.emotion = result.emotion;
        co.metadata.subject = result.subject;
        co.addStep({ name: 'audio_transcribed' });
      } else {
        co.summary = result?.error || 'Transcription indisponible';
        co.addStep({ name: 'transcription_error', note: result?.error });
      }
    } catch (err) {
      co.summary = 'Erreur transcription audio';
      co.addStep({ name: 'transcription_error', error: err.message });
    }
  }
