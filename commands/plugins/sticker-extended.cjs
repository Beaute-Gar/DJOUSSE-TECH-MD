...
cmd({
    ...
    try {
        ...
        if (type === 'imageMessage') {
            ...
            return reply(box('STICKER', ['SEND OR REPLY TO AN IMAGE OR VIDEO/GIF (MAX 10S)']));
        }
        ...
        const author = config.OWNER_NAME || 'DJOUSSE TECH';
        webpBuffer = await addExif(webpBuffer, config.BOT_NAME || 'DJOUSSE-BOT', author);

        await conn.sendMessage(from, { sticker: webpBuffer }, { quoted: m });
        await conn.sendMessage(from, { react: { text: '✅', key: m.key } });
...
cmd({
    ...
    category: 'tools',
    filename: __filename,
}, async (conn, m, commands, { from, args, reply }) => {
    try {
        const quoted = m.quoted ? m.quoted.message : null;
        if (!quoted || !quoted.stickerMessage) {
            return reply(box('TAKE', ['REPLY TO A STICKER WITH .take TO REUSE IT.', '', 'EXAMPLE: take MyPack']));
        }

        const input = args.join(' ');
        const packname = input.trim() || config.BOT_NAME || 'DJOUSSE-BOT';
        const author = config.OWNER_NAME || 'DJOUSSE TECH';
        ...
        reply('❌ Error processing sticker.');
    }
});
