...
const logger = {
    ...
    memory(rssMB, heapMB, heapTotalMB) {
        ...
        if (level !== this._lastMemLevel) {
            ...
            switch (level) {
                case 'normal': _term('green', `🟢 Mémoire normale — RSS ${rssMB.toFixed(0)} MB`); break;
                case 'warning': _term('yellow', `🟡 Pression mémoire — RSS ${rssMB.toFixed(0)} MB`); break;
                case 'elevated': _term('yellow', `🟠 Mémoire élevée — RSS ${rssMB.toFixed(0)} MB`); break;
                case 'high': _term('red', `🔴 ALERTE MÉMOIRE — RSS ${rssMB.toFixed(0)} MB`); break;
                case 'critical': _term('red', `🚨 MÉMOIRE CRITIQUE — RSS ${rssMB.toFixed(0)} MB`); break;
            }
        }
    },
