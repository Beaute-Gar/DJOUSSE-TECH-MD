...
if (!buffer) return m.reply(boxWithFooter('ERREUR', [{ raw: '❌ Erreur téléchargement image.' }]));
await conn.updateProfilePicture(m.chat, buffer);
return m.reply(boxWithFooter('🖼️ *PHOTO DE GROUPE*', [
  { label: 'Statut', value: '✅ Photo mise à jour' },
]));
}
m.reply(boxWithFooter('🖼️ *PHOTO DE GROUPE*', [
