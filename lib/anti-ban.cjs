    ...
    while (messageQueue.length > 0) {
        ...
        if (!check.ok) {
            if (check.reason === 'restricted') {
                ...
                for (const pending of messageQueue) pending._settle?.(new Error('restricted'));
                messageQueue = [];
                break;
            }
...
function queueMessage(fn) {
    return new Promise((resolve, reject) => {
        ...
        task._settle = (err) => err ? reject(err) : resolve(undefined);
        messageQueue.push(task);
        processQueue();
    }).catch(() => false); // never rejects — callers can safely .catch() or await
}
...
function destroy() {
    if (dailyResetTimer) clearInterval(dailyResetTimer);
    for (const pending of messageQueue) pending._settle?.(new Error('destroyed'));
    messageQueue = [];
    sock = null;
}
