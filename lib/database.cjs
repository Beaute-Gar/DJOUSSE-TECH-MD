...
// ═══════════════════════════════════════════════════════════════════════════
//  SESSIONS (Baileys handles auth — these track active numbers)
// ═══════════════════════════════════════════════════════════════════════════
async function saveSessionToMongoDB(number, credentials) {
    ...
    try {
        if (USE_MONGO) {
            ...
            const db = readJSON('sessions');
            db[clean] = { credentials, updatedAt: new Date().toISOString() };
            writeJSON('sessions', db);
        }
        return true;
    } catch (error) {
...
async function getSessionFromMongoDB(number) {
    ...
    try {
        if (USE_MONGO) {
            ...
            const db = readJSON('sessions');
            return db[clean] ? db[clean].credentials : null;
        }
    } catch (error) {
...
async function deleteSessionFromMongoDB(number) {
    ...
    try {
        if (USE_MONGO) {
            ...
            delete db[clean];
            writeJSON('sessions', db);
            const an = readJSON('active_numbers');
            delete an[clean];
            writeJSON('active_numbers', an);
        }
        return true;
    } catch (error) {
