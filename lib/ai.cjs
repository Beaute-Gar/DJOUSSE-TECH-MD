// lib/ai.cjs — Moteur IA DJOUSSE TECH (Groq) — modèles officiels du bot
// Chat/raisonnement: openai/gpt-oss-120b · Rapide: openai/gpt-oss-20b
// Multilingue/Vision: qwen/qwen3.6-27b · Safety: openai/gpt-oss-safeguard-20b
// STT: whisper-large-v3 · TTS: Google (fr) — Orpheus (en/ar) non utilisé pour le français
...
async function groq(messages, { model, temperature = 0.7, maxTokens = 2048, json = false, timeout = 40000 } = {}) {
  ...
  if (/gpt-oss|reasoning/i.test(body.model)) body.reasoning_effort = 'medium';
...
/** Chat principal — gpt-oss-120b avec repli automatique (20b → qwen → pollinations → prexzy) */
...
/** Chaîne IA gratuite de bout en bout : GROQ → pollinations (sans clé) → prexzy (gratuit) */
async function freeChat(text, { system, temperature, context } = {}) {
  ...
  try {
    const r = await pollinations(system ? system + '\n\nQuestion : ' + String(text) : String(text));
    if (r && r.length) return r;
...
/** Vision — qwen/qwen3.6-27b (image + question) */
...
/** Modération IA — openai/gpt-oss-safeguard-20b */
