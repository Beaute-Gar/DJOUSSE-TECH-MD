...
async function printGlitchTitle() {
    const art = renderTitle('DJOUSSE') + '\n' + renderTitle('TECH');
    const lines = art.split('\n');

    // Passage 1 : silhouette magenta décalée (effet fantôme)
    for (const l of lines) console.log(`${MG}${DIM}${l}${R}`);
    await sleep(120);
    // Passage 2 : le vrai titre en vert vif
    console.log('');
    for (const l of lines) console.log(`${BG}${B}${l}${R}`);
}
...
module.exports = { printBootSequence, matrixRain, progressBar, securityScan, printGlitchTitle };
