...
async function chat(text, { system, temperature = 0.7, context } = {}) {
  ...
  // Injecter l'identité dans le user message aussi (certains modèles ignorent le system)
  const identityReminder = '[AINORIA — DJOUSSE TECH — Beauté Gar — Cameroun]\n';
  messages.push({ role: 'user', content: identityReminder + String(text || '') });
