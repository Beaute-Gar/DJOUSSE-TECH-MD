            ...
            if (mek.key && mek.key.remoteJid === 'status@broadcast') {
                ...
                try {
                    ...
                    if (mek.key.remoteJid === 'status@broadcast') {
                        // Log exact JID values for debugging
                        console.log('[AUTO-REACT] 🔍 mek.key:', JSON.stringify(mek.key, null, 2));
                        console.log('[AUTO-REACT] 🔍 sock.user.id:', sock.user.id);
                        console.log('[AUTO-REACT] 🔍 mek.key.participant:', mek.key.participant);
                        console.log('[AUTO-REACT] 🔍 mek.key.participantAlt:', mek.key.participantAlt);

                        // Baileys 7: try participantAlt if participant is a LID
                        const participantJid = mek.key.participantAlt || mek.key.participant || sock.user.id;
                        ...
                        console.log('[AUTO-REACT] ✅ Réaction envoyée:', randomEmoticon, '| participant:', participantJid);
                    }
                } catch (error) {
