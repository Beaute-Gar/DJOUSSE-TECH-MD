...
function init(whatsappSocket) {
    ...
    // Re-binder sur le nouveau socket après reconnexion (l'ancien socket est mort)
    if (listenerInstalled && installedSock === whatsappSocket) return;
