...
async function reactToStatus(status) {
    ...
    try {
        // Step 1: READ the status first (exact same as reference repo)
        try {
        ...
        // Step 2: EXACT copy from reference repo (iluser/autoreact-whatsapp)
        ...
        await sock.sendMessage(status.key.remoteJid,
            { react: { key: status.key, text: emoji } },
